resource "snowflake_stage" "app_stage" {
  ...
  name     = "${each.value.app_folder_name}_STAGE"
...
resource "snowflake_streamlit" "streamlit_app" {
  for_each = { for app in var.apps : app.app_name => app }
  name            = "${each.value.app_folder_name}_STREAMLIT"
  database        = each.value.database
  schema          = each.value.schema
...
resource "null_resource" "upload_streamlit_script" {
  ...
  provisioner "local-exec" {
    ...
    command = "snowsql -a ${var.snowflake_org_name}-${var.snowflake_account_name} -u ${var.snowflake_user} -r ${var.snowflake_role} -d ${each.value.database} -s ${each.value.schema} -f ../apps/${each.value.app_folder_name}/upload_app.sql"
  }
